...
function readmeRows(): string[][] {
  return [
    ...
    ['- If you have columns of your own, a deleted product leaves its row blank in place rather than shifting the rows below, so your notes and formulas stay aligned. A blank row is ignored on the next Pull.'],
    ['- With no columns of your own, the catalogue closes the gap up instead, and the only thing lost is a formula on a row that genuinely moved.'],
